...
from eval import *
from model import *
from util_f import MyDynamicDataset1,MyDynamicDataset,load_k_fold


# Arguments
parser = argparse.ArgumentParser(description='DVGEDR')
...
def extract_subgraph(split_data):
    if args.data_name == 'Gdataset':
        ...
        (
            adj_drug, adj_disease, adj_train, train_labels, train_u_indices, train_v_indices,
    ...
    train_graph1 = MyDynamicDataset1(train_file_path, adj_drug, adj_disease, train_indices,
                                                 train_labels, args.hop)
    # train_graph_dgl = MyDynamicDataset_dgl(train_file_path, adj_train, train_indices, train_labels, args.hop)
    ...
    test_graph1 = MyDynamicDataset1(test_file_path, adj_drug, adj_disease, test_indices, test_labels, args.hop)
    # test_graph_dgl = MyDynamicDataset_dgl(test_file_path, adj_train, test_indices, test_labels, args.hop)
    return train_graph,train_graph1, test_graph, test_graph1
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        print("============= seed=", str(seed), "==================")
        split_data_dict = load_k_fold(args.data_name, seed, neighbor_G)

        for k in range(0, 10):
            print('------------ fold', str(k + 1), '--------------')
            train_graphs,train_graph1, test_graphs,test_graph1 = extract_subgraph(split_data_dict[k])
            model = DVGEDR(
                train_graphs,
                train_graph1,
                latent_dim=[64, 64, 64],
                ...
                force_undirected=args.force_undirected, device=args.device
            )
            ...
            auroc, aupr = train_epochs(train_graphs,train_graph1, test_graphs,test_graph1, model, args, seed)
